...
- (NSData *)encrypt:(NSData *)key
  ...
  return [self encrypt:key
           withInitial:nil andPadding:options];
}
...
+ (NSData *)dataWithBase32String:(NSString *)encoded {
  ...
  unsigned long charsLen = [encoded lengthOfBytesUsingEncoding:NSASCIIStringEncoding];
  ...
  unsigned long bytesLen = (charsLen * 5) >> 3;
  Byte bytes[bytesLen];
...
- (NSString *)base32String {
  ...
  unsigned long bytesOffset = 0, bytesLen = [self length];
  unsigned long charsOffset = 0, charsLen = ((bytesLen << 3) + 4) / 5;
  char chars[charsLen];
  while (bytesLen != 0) {
    int digit, lastDigit;
  ...
  return [NSString stringWithCString:chars
                            encoding:NSUTF8StringEncoding];// stringWithCString:chars length:sizeof(chars)];
}
...
- (unsigned int)crc32 {
  ...
  return (unsigned int)(crcval ^ 0xffffffff);
}
...
- (NSData *)sha1Digest {
  HEComputeDigestNSData(SHA);
}

- (NSString *)sha1DigestString {
  HEComputeDigestNSString(SHA);
}
